    ...
    const fetchSellerData = async () => {
      try {
        ...
        const foldersList = await Promise.all(foldersSnapshot.docs.map(async (folderDoc) => {
          ...
          // Count cards to display on folder
          const cardsSnapshot = await getDocs(collection(db, `folders/${folderId}/cards`));
    ...
    <div className="w-full bg-transparent flex flex-col min-h-screen">
      <div className="w-full max-w-[1600px] mx-auto xl:px-12 2xl:px-16 animate-[fadeIn_0.5s_ease-out] flex-1 flex flex-col">
        ...
        <div className="relative border-b border-gray-200 md:border-x shadow-sm py-8 px-6 md:px-12 flex flex-col md:flex-row items-center md:items-start gap-8 overflow-hidden bg-white">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8 w-full relative z-20">
            {/* Avatar */}
            <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white shadow-xl bg-white flex items-center justify-center overflow-hidden flex-shrink-0 relative group z-20">
            ...
            {/* Seller Info */}
            <div 
              className={"text-center md:text-left mt-2 w-full md:w-fit md:max-w-3xl transition-all " + 
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-4">
                ...
                {(() => {
                  ...
                  if (defaultAddress) {
                    ...
                    return (
                      <span className={`flex items-center gap-1.5 ${hasStreet ? 'bg-blue-50 border-blue-200 text-blue-800' : 'bg-gray-50 border-gray-200 text-gray-700'} px-4 py-2 rounded-full border shadow-sm text-sm font-semibold`} title={hasStreet ? "Dirección de la tienda" : "Ubicación"}>
                        <span className="material-symbols-outlined text-[18px]">{hasStreet ? 'storefront' : 'location_on'}</span>
                        {displayText}
                      </span>
                    );
                  }
                  if (seller?.region || seller?.comuna) {
                    return (
                      <span className="flex items-center gap-1.5 bg-gray-50 px-4 py-2 rounded-full border border-gray-200 text-gray-700 shadow-sm text-sm font-semibold">
                ...
                {/* Instagram */}
                {seller?.instagramUrl && (
                  <a href={`https://instagram.com/${seller.instagramUrl.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-pink-50 hover:bg-pink-100 text-pink-600 px-4 py-2 rounded-full border border-pink-200 shadow-sm transition-colors cursor-pointer text-sm font-bold">
            ...
            <div className="mb-12 mx-auto w-full max-w-[1200px]">
              ...
              {folders.length === 0 ? (
                <div className="bg-white rounded-2xl p-12 text-center border border-dashed border-blue-300 shadow-sm">
                  <span className="material-symbols-outlined text-6xl text-blue-300 mb-4">inventory_2</span>
