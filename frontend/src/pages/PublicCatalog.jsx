...
function PublicCatalog() {
  ...
  useEffect(() => {
    ...
    const fetchCatalogData = async () => {
      try {
        ...
        setErrorMsg("Hubo un error al cargar el catÃƒÂ¡logo.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleWhatsAppCheckout = async () => {
    ...
    if (!phone) {
      alert("El vendedor no tiene un nÃƒÂºmero de contacto configurado.");
      return;
    }
    
    const formattedPhone = phone.startsWith('56') ? phone : `56${phone}`;
    
    // Abrir ventana sÃƒÂ­ncronamente para evitar bloqueo de pop-ups
    ...
    try {
      ...
      await api.post('/orders/create', {
        ...
        folderId: folderId,
        folderName: folderData.name || 'CatÃƒÂ¡logo',
        items: cart.map(item => ({
          id: item.id,
          name: item.name,
      ...
      let message = `Ã‚Â¡Hola! Vengo de Carpetazo. Me interesa comprar estas cartas de la carpeta "${folderData?.name || 'CatÃƒÂ¡logo'}":\n\n`;
      cart.forEach(item => {
        message += `ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¢ ${item.quantity}x ${item.name} (${item.set}) - ${formatCLP(item.price * item.quantity)}\n`;
      });
      message += `\nTotal: ${formatCLP(cartTotal)}\n\nÃ‚Â¿Tienes disponibilidad?`;
      ...
      if (newWindow) {
        ...
        window.location.href = whatsappUrl; // fallback si el navegador bloquea incluso el window.open sÃƒÂ­ncrono
      }
  ...
  const counts = {
    pokemon: cards.filter(c => c.supertype === 'PokÃƒÂ©mon').length,
      ...
      <div className="relative border-b border-gray-200 md:border-x shadow-sm py-6 px-6 md:px-12 flex flex-col md:flex-row justify-between items-center gap-6 overflow-hidden bg-white">
            ...
            <div className="flex flex-col text-left flex-1 min-w-0">
              <div className="flex items-center gap-1 flex-wrap">
                <span className="text-base font-black text-[#1a2b4b] leading-tight">
                  {sellerData?.displayName || 'Vendedor AnÃƒÂ³nimo'}
                </span>
              ...
              <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                {(sellerData?.totalTrades > 0) ? (
                  <>
                    ...
                    <span className="text-gray-500 text-[10px] font-semibold">{sellerData?.totalTrades} reseÃƒÂ±as</span>
                  </>
                ) : (
        ...
        <div className="relative z-10 flex flex-col items-start md:items-end gap-3 w-full md:w-auto">
          ...
          <Link to={`/${sellerData?.username || folderData.userId}`} className="flex items-center gap-1.5 text-xs font-bold text-[#1e40af] hover:text-blue-800 transition-colors group mt-2 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-xl border border-gray-200 shadow-sm">
            <span>Ver catÃƒÂ¡logo completo del vendedor</span>
        ...
        <div className="w-full overflow-hidden shadow-[0_30px_60px_-15px_rgba(0,0,0,0.6)] md:border-x md:border-b border-gray-300 flex flex-col relative z-10 min-h-[calc(100vh-200px)] bg-[#DBEAFE]">
            ...
            <div className="flex flex-col md:flex-row gap-2">
              <div className="flex-1 relative">
                ...
                <input 
                  ...
                  placeholder="Buscar nombre de carta o nÃƒÂºmero..."
          ...
          <div className="flex justify-end mb-4 border-b border-gray-100 pb-4">
            <div className="bg-gray-100 p-1 rounded-xl flex items-center shadow-inner">
              ...
              >
                <span translate="no" className="material-symbols-outlined text-lg">auto_stories</span> ÃƒÂlbum
              </button>
              <button 
              ...
              >
                <span translate="no" className="material-symbols-outlined text-lg">grid_view</span> CuadrÃƒÂ­cula
              </button>
            </div>
          </div>
          ...
          {filteredCards.length === 0 ? (
            <div className="py-12 text-center text-gray-500 flex flex-col items-center">
                ...
                <p>Este catÃƒÂ¡logo aÃƒÂºn no tiene cartas o no coinciden con tu bÃƒÂºsqueda.</p>
            </div>
          ) : viewMode === 'album' ? (
            <AlbumView 
              cards={filteredCards} 
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] flex justify-end" onClick={() => setIsCartOpen(false)}>
          <div className="bg-white border-l border-gray-200 w-full max-w-md h-full p-6 flex flex-col shadow-2xl animate-[slideIn_0.3s_ease_forwards]" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-6 pt-4 border-t border-gray-200">
              ...
              <p className="text-[10px] text-center text-gray-500 mt-3">Al presionar, se abrirÃƒÂ¡ WhatsApp con el detalle de tu pedido para coordinar el pago y envÃƒÂ­o directamente con el vendedor.</p>
            </div>
          </div>
        </div>
      )}
