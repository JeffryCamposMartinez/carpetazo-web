...
seller_profile = r'C:\Users\Jeffry\Desktop\Carpetazo.cl\Publicar mis cartas\frontend\src\pages\SellerProfile.jsx'
profile_page = r'C:\Users\Jeffry\Desktop\Carpetazo.cl\Publicar mis cartas\frontend\src\pages\ProfilePage.jsx'

old_seller = '''  const handleImageUpload = async (event, type) => {
...
new_seller = '''  const handleImageUpload = async (event, type) => {
    const file = event.target.files?.[0];
    if (!file || !isOwner) return;
    if (!file.type.startsWith('image/')) return alert('Sube una imagen válida.');
    if (file.size > 10 * 1024 * 1024) return alert('La imagen es demasiado grande. Máximo 10MB.');
...
replace_in_file(seller_profile, old_seller, new_seller)

old_profile = '''  const handleAvatarUpload = (event) => {
    const file = event.target.files?.[0];
    if (!file) return;
...
new_profile = '''  const handleAvatarUpload = async (event) => {
    const file = event.target.files?.[0];
    if (!file) return;
    if (!file.type.startsWith('image/')) return showFeedback('error', 'Sube una imagen válida.');
...
replace_in_file(profile_page, old_profile, new_profile)
