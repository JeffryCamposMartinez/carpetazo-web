...
const oldCatch = `  } catch (err) {
    ...
    isDownloading = false;
    sendEmail('?? TCG Master Downloader: Se ha detenido por un error interno: ' + err.message);
  }`;
...
const newCatch = `  } catch (err) {
    ...
    sendEmail('?? TCG Master Downloader: Se detect un bloqueo largo de seguridad (ej. Cloudflare). El sistema se pondr a dormir y auto-reintentar en 1 hora.');
    
    // Auto-reinicio en 1 hora
    setTimeout(() => {
        console.log("Despertando tras 1 hora de reposo, reintentando extraccin...");
        if (!isDownloading) {
            startDownloadEngine();
        }
    }, 60 * 60 * 1000);
...
if (serverCode.includes(oldCatch)) {
    serverCode = serverCode.replace(oldCatch, newCatch);
} else {
    console.log("No se encontro el bloque catch original en server.js");
}
...
const oldIf = `if (data.isWaitingSync) {
...
const newIf = `if (data.isWaitingSync) {
                badge.innerHTML = '<span class="w-2 h-2 rounded-full bg-blue-400 animate-pulse"></span> CICLO 24H (Sincronizado)';
                badge.className = 'px-4 py-1.5 rounded-full text-sm font-bold bg-blue-900/50 text-blue-300 border border-blue-700/50 flex items-center gap-2';
                document.getElementById('btnStart').disabled = true;
                document.getElementById('btnStop').disabled = false;
...
if (htmlCode.includes(oldIf) && !htmlCode.includes("data.isWaitingCloudflare")) {
    htmlCode = htmlCode.replace(oldIf, newIf);
    fs.writeFileSync("public/index.html", htmlCode);
} else {
    console.log("No se encontro el if original en index.html");
}
