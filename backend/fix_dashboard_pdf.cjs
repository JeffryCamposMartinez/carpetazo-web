...
// 3. Add Button
// The edit button has title="Renombrar carpeta"
if (!code.includes("picture_as_pdf")) {
  const replaceButton = `<button 
              ...
              <button`;
  code = code.replace(/<button[^>]*title="Renombrar carpeta"[^>]*>[\s\S]*?<\/button>/, match => replaceButton.replace('<button', match));
  // Wait, I messed up the replacement logic. I should just prepend the button to the match.
}

// 4. Add Loading Modal
if (!code.includes("HiddenPDFGenerator folderId={generatingPdfFolder}")) {
  const modalHTML = `
...
`;
  code = code.replace(/\{\/\*\s*Folder Deletion Modal\s*\*\/\}/, match => modalHTML + match);
}
