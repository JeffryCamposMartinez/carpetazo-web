...
const cleanProductName = (prod) => {
  ...
  if (prod === 'primer bloque') return 'Otros';
  return prod;
};
...
async function main() {
  ...
  let nextGroupId = 999200; // NEW OFFSET
  ...
  // Delete Old Groups
  const oldGroups = await prisma.tcgGroup.findMany({ where: { categoryId: 99, groupId: { lt: 999200 } } });
