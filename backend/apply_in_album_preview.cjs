...
// 1. Remove the entire tooltip block from renderPocket.
// We use a robust regex that matches the div containing the tooltip PosClass until it closes.
code = code.replace(/<div className=\{`absolute top-\[95%\].*?\{renderCardActions\(card\)\}\s*<\/div>\s*\)\}\s*<\/div>/s, "");

// 2. Change the click handler to also set previewCard
code = code.replace(
  "setActiveCardId(cardIsActive ? null : uniqueId);",
  "setActiveCardId(cardIsActive ? null : uniqueId);\n                        setPreviewCard(card);"
);

// 3. Inject previewCard state at the top
if (!code.includes("const [previewCard, setPreviewCard] = useState(null);")) {
    code = code.replace(
...
// 4. Inject the In-Album Spread Preview using a function to avoid Javascript $ replacement corruption!
...
console.log("Successfully safely applied the in-album preview spread");
