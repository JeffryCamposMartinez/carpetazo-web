const fs = require('fs');
let code = fs.readFileSync('C:/Users/Jeffry/Desktop/Carpetazo.cl/Publicar mis cartas/frontend/src/pages/Dashboard.jsx', 'utf8');

// Use regex to completely rip out the misplaced modal chunk
const chunkRegex = /\s*\{\/\* PDF Generator Components \*\/\}[\s\S]*?\{generatingPdfFolder && \([\s\S]*?<\/div>\s*\)\}\s*/;
...
const targetEndRegex = /\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/>\s*\);\s*}\s*$/;
code = code.replace(targetEndRegex, "\n" + modalHTML + "\n    </div>\n      </div>\n      </div>\n    </>\n  );\n}");

fs.writeFileSync('C:/Users/Jeffry/Desktop/Carpetazo.cl/Publicar mis cartas/frontend/src/pages/Dashboard.jsx', code);
console.log("Successfully fixed the injection mess!");
